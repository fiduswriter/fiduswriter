...
var tmp_comments = _.template(
    '<% _.each(theComments,function(comment,key,list){ %>\
        ...
        class="comment-box \
            <% if(comment.id===theEditor.pm.mod.comments.layout.activeCommentId) { %>active<% } else { %>inactive<% } %>\
            <% if(comment["review:isMajor"] === true) { %>comment-is-major-bgc<% }%>\
            " style="top:<%= theEditor.pm.mod.comments.layout.calculateCommentBoxOffset(comment) %>px;">\
            <% if (comment.id===theEditor.pm.mod.comments.layout.activeCommentId || comment.comment.length > 0) { %>\
            <% if(0 === comment.comment.length) { %>\
                <%= tmp_first_comment({"comment": comment}) %>\
            <% } else { %>\
            <%= tmp_single_comment({"comment": comment, active: (comment.id===theEditor.pm.mod.comments.layout.activeCommentId)}) %>\
            <% } %>\
            <% if (comment.answers && comment.answers.length) {\
               for (var i=0;i < comment.answers.length; i++) { %>\
                         <%= tmp_answer_comment({comment: comment, answer: comment.answers[i], active: (comment.id===theEditor.pm.mod.comments.layout.activeCommentId)}) %>\
             <% }\
            } %>\
            <% if(comment.id===theEditor.pm.mod.comments.layout.activeCommentId && 0 < comment.comment.length) { %>\
    ...
    '</button>\
            ...
            <% } %>\
            <% if(comment.id===theEditor.pm.mod.comments.layout.activeCommentId && (comment.user===theUser.id || theDocument.is_owner)) { %>\
...
var tmp_answer_comment = _.template(
    '<div class="comment-item">\
        ...
        <% if (active && answer.id===theEditor.pm.mod.comments.layout.activeCommentAnswerId) { %>\
            <div class="comment-text-wrapper">\
