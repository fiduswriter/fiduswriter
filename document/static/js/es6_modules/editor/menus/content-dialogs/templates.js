...
export let sowidaraTemplate = _.template('\
    ...
    <div class="item">\
        <h3 style="border-top: 1px solid #d3d3d3;margin-bottom:5px;margin-top:5px;" ><a class="title" id = <%=item.id %> itemTitle = <%=item.title %>  itemAuthor = <%=_.first(item.person_author_normalized_str_mv) %>  itemDate = <%= item.date_imported_str %>  target="_blank" href="http://www.da-ra.de/dara/search/search_show?res_id=<%=item.id %>&lang=de&mdlang=de&detail=true"><%= item.title %></a></h3>\
