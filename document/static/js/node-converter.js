...
(function () {
    ...
    nodeConverter.toModel = function (viewNode) {
        var modelNode = viewNode.cloneNode(true), agNodes = modelNode.querySelectorAll('.citation, .equation'), fnNodes, newFn,
    ...
    /** Specifies a node to insert before each footnote node. */
    nodeConverter.beforeFootnote = function () {
        return document.createTextNode('\u180e');
    };
    
    /** Specifies a node to insert after each complex node. */
    ...
    nodeConverter.cleanNodeView = function (node) {
         var node = node.cloneNode(true), agNodes = node.querySelectorAll('.citation,.equation'),
            i;
        for (i = 0; i < agNodes.length; i++) {
            while (agNodes[i].firstChild) {
                agNodes[i].removeChild(agNodes[i].firstChild);
