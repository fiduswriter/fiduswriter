...
//       return;
    ...
    serverCommunications.resetTextChangeList = function () {
        ...
        theDocumentValues.diffNode = cleanAGNode(document.getElementById('document-editable').cloneNode(true)); // a node against which changes are tested constantly.
    };

    serverCommunications.addCurrentToTextChangeList = function () {
        theDocumentValues.textChangeList.push([cleanAGNode(document.getElementById('document-editable').cloneNode(true)), new Date().getTime() + window.clientOffsetTime]);
    };


    serverCommunications.makeDiff = function () {
        var theDiff = domDiff.diff(theDocumentValues.diffNode, cleanAGNode(document.getElementById('document-editable').cloneNode(true))),
            containsCitation = 0,
            containsEquation = 0,
            containsComment = 0,
            diffText = '',
    ...
    serverCommunications.orderAndApplyChanges = function () {
        ...
        tempPatchedNode = cleanAGNode(theDocumentValues.textChangeList[theDocumentValues.textChangeList.length - 1][0].cloneNode(true));
        ...
        applicableDiffs = domDiff.diff(cleanAGNode(document.getElementById('document-editable').cloneNode(true)), tempPatchedNode);
