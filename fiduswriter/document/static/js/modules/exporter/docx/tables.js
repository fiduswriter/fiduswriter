...
export class DOCXExporterTables {
    ...
    addTableNormalStyle() {
        ...
        const tableNormalEl = this.styleXml.getElementByTagNameAndAttributes("w:style", {"w:type": "table", "w:default": "1"})
        if (tableNormalEl) {
    ...
    addTableGridStyle() {
        ...
        const tableGridEl = this.styleXml.getElementByTagNameAndAttributes("w:style", {"w:type": "table", "w:customStyle": "1"})
        if (tableGridEl) {
    ...
    getSideMargins() {
        if (!this.sideMargins) {
            const marginsEl = this.styleXml.getElementByTagNameAndAttribute("w:style", "w:styleId", this.tableGridStyle)
            const leftEl = marginsEl.getElementByTagName("w:left")
            const rightEl = marginsEl.getElementByTagName("w:right")
