...
export let journalDialogTemplate = _.template('\
    ...
    <%  _.each(journals, function(journal, index) {%>\
        <div class="fw-radio">\
            <input type="radio" id="journal_<%= journal.ojs_jid %>" name="journalList" value="<%= journal.ojs_jid %>" <% if (index===0) { %>checked="checked"<% } %>>\
            <label for="journal_<%= journal.ojs_jid %>"><%= journal.name %></label>\
        </div>\
   <% })  %> \
</div>\
')
